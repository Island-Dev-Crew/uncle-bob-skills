...
- Preserved the next successor receipt at commit `1f6040f` / tree `b457503f` as **CHANGES REQUIRED** too. It showed that Bash's alternate `function name { ...; }` grammar still forged a proof inside an annotated compound; that exact spelling led to watched variants across command positions, arrays and conditional data, executable substitutions, legacy backticks, literal Bash/sh `-c` source, comments, locale-quoted argv, wrappers and redirections, dynamic evaluators, imported functions, stdin-fed shells or runtime-computed shell script operands, and conservatively refused child heredocs. The static argv reader now retains quote/escape provenance with a reversible escaped-marker grammar, so active patterns and substitutions cannot hide a generated script while the same punctuation quoted as literal data remains admissible; literal private-use Unicode—including the complete private-use ranges—cannot collide with or exhaust those markers. The one shipped Gherkin transcript that assembled `bash -c` source through variables now states its equivalent literal paths. The same receipt found two public evidence errors: three preserved stills were absent from the claimed frame-by-frame map, and mutable repository metadata had been mistaken for August code activity across all three `crap4*` repositories. The extraction record now links every preserved still directly, and the research/skill claims record the three latest default-branch commit dates observed on 2026-08-26 instead of laundering `updated_at` into code activity. Publication still requires a new accepting receipt at the final head and tree.
- Preserved the third successor receipt at commit `a3d0572` / tree `d0ab0675` as **CHANGES REQUIRED**. It found that a literal `xargs bash -c` child could hide the same function-shadowing proof forgery from both certifiers, and that valid repeated unmatched shell-data words drove the masker at approximately quadratic cost. The repair now parses an explicitly tested BSD/GNU `xargs` option union, inspects an already-fixed literal child-shell source, and fails closed when appended or replacement-built input can determine launcher options, source, startup files, or a wrapper executable; inert `printf` and `dirname` leaves remain admissible. Combined Bash option clusters now respect each `-O`/`-o` value before locating `-c`, while no-exec controls stay admissible. Brace-expansion classification preserves distinct quote and escape provenance for commas, dots, signs, digits, and endpoints, then follows the first syntactically active candidate under fixed semantic probes of the exact absolute Bash and sh selected at startup. That distinction is watched at both edges: a malformed active sequence can block a later candidate on legacy Bash, while a protected dot that never formed the `..` delimiter and a nested comma that outranks a selected malformed outer sequence must not hide argv generation that rewrites an `xargs` replacement marker; sequence increments are admitted only when the measured interpreter supports them, including validated Bash's retained outer braces around nested-comma replacement words. The probes are byte-decoded, structurally complete, and share a startup-hook/function/search-path-scrubbed environment with replay. A bare or exact startup-bound child inherits its own measured Bash-or-sh profile—including Bash's real brace behavior when invoked as `sh`—while a slash-qualified relative path or alternate absolute shell is inspected under the conservative cross-release union; no child is executed merely to identify it. The same source boundary now refuses executable Bash/sh rcfiles, init files, debugger and interactive/login profiles, unsafe leading or `env`-wrapped lookup/startup assignments, ambiguous `env` path/chdir/argv0/split-string grammar, and child source hidden behind trailing `nohup` data. Delimiter matching and command-prefix recognition now use precomputed closer tables and finite-state scans, preserving the former accepted grammar across fixed-seed equivalence suites while keeping every watched 32K near miss linear. Publication still requires a fresh accepting receipt bound to the final successor head and tree.
