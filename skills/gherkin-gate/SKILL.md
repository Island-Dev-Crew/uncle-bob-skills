...
An open box means the verdict stays `unverified`. Repair by running the missing phase, re-hashing after a scenario edit, or fixing the code. Then re-run the gate and re-check the boxes; the loop ends only when the tool consents (C4).
