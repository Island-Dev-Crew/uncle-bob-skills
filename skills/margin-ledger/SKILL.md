...
- **Enforced today**: the arithmetic and the floor verdict. [`scripts/margin-ledger.py`](scripts/margin-ledger.py) computes margins exactly, exits 1 on a floor breach, exits 2 fail-closed on an empty or malformed ledger — a `nan` or `inf` minute included, since every comparison is False against NaN and such a row used to print WIDE at exit 0 — and uses that same pack-wide 2 seal when its report cannot be written. It exits 3 on an unreadable input ledger or bad invocation, a non-finite `--floor` among them, which disabled the breach test wholesale. Input-path/usage 3 stays distinct from malformed-content 2: while both were 2, running from the wrong directory produced a path error that read exactly like a legitimate fail-closed content result. Argparse's own failures are translated to 3 for the same reason — an unknown option, a missing `--floor` value and a `--floor abc` all exited argparse's 2, so a mistyped command was indistinguishable from malformed content; `--help` is not a failure and still exits 0. Exit 2 also covers minutes that sum past float range: a gated column overflowing to `inf` divides a finite human total to 0.00x, and the gate printed LOST at exit 1 over rows that each scored in band. That breach was manufactured by floating point, so the sums are now checked before the division rather than the quotient after it. The island's own shape is enforced by the pack validator (`scripts/validate-island.py` at the pack root).
