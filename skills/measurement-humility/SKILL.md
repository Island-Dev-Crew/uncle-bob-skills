...
**Red/green proof.** All seven fixtures ship beside the script. Recompute from this island's directory rather than trusting these lines:
...
**Each breach verdict is load-bearing on its own.** The pack verifier compares exit codes, never verdict text, and `theatre.tsv` trips every breach at once, so any one verdict could stop being emitted there and its exit 1 would survive on the neighbours. Each breach verdict therefore also has a run where it is the only thing wrong, so that exit 1 rests on that verdict alone. `UNGUARDED` has `punctuation-evasion.tsv` above: five rows, all `UNGUARDED`, no `DUE`, no `DUPLICATE`. `DUE` has `pack-metrics.tsv` read on `2027-06-01` below: five rows, all `DUE`. `DUPLICATE` has this one:

```bash
python3 scripts/metric-register.py --today 2026-08-21 scripts/fixtures/duplicate-only.tsv  # exit 1 — DUPLICATE is the only breach: one clean row and its respelled twin
```

Two rows, `1 reviewed, 0 unguarded, 0 due, 1 duplicate`. Watched failing before it was trusted: with the `DUPLICATE` arm of `judge()` disabled, this file exits 0 and the verifier reports the mismatch, while the same edit left `theatre.tsv` at exit 1 and the verifier at `0 mismatched`. The clean verdict is covered the other way round: `REVIEWED` is the `else` arm, and removing its assignment turns every exit-0 run here into an internal failure, exit 2, which the verifier also reports.
