...
The hardener seat is "absolutely merciless" (C9): a surviving mutant "must be killed" (C7). The ledger exists because mercilessness meets a hard limit — detecting mutant equivalence is undecidable (Budd & Angluin 1982), and 4–39% of mutants are equivalent in practice, so a literal 100% kill score is provably unreachable (grounding: [research/mutation-testing.md](../../research/mutation-testing.md)). The honest 100% is **killed + argued-equivalent, nothing unexplained**: every survivor either gets a killing test or a written excusal here, and an unexcused survivor holds the gate red (C4) until one of the two happens.

## The anti-laundering rule

An excusal is an **equivalence claim**. "Could not kill it" describes the author's effort; "equivalent" describes the program. Only the second may enter the ledger, and it must be argued: the `argument` field states why *no possible test* can observe the mutation — the mutated program computes the same function (a redundant bounds check the type system already guarantees, a boundary the loop body makes unreachable, an operand swap under commutativity). A survivor you failed to kill but cannot argue equivalent stays a survivor: it goes back to the hunt as a kill-task, or it blocks. There is no third state.
...
Keep the ledger beside the run's captured output — `evidence/<story-slug>/out/excusals.md` — one entry per excused mutant:
...
The id is whatever the mutation tool emits (a PIT mutator+line, a Stryker id) — the one contract is that it matches the survivor list byte-for-byte. `excused-by` names a seat (`OpenAI Codex`, `Claude Fable 5`, `Jon Isaac` — [repo law](../../CONTEXT.md)); `head` pins the ruling to the exact code it examined, because an equivalence argument about code that has since moved is void.

## The gate

[`scripts/check-excusals.py`](scripts/check-excusals.py) takes the survivor list (one id per line, as the hunt emits it) and the ledger, and exits non-zero on any survivor with no entry, an entry missing a required field, or an argument under 40 characters:
...
The loop: run the gate → for each FAIL, either write a real equivalence argument or send the mutant back to the hunt as a kill-task, removing it from the survivor list only when the re-run hunt confirms the kill → re-run the gate until exit 0. Stale excusals (entries for mutants no longer surviving) warn — prune them; sediment in a ledger is how laundering starts.
...
- **Advisory** (honestly, unavoidably): the *truth* of an equivalence argument. Undecidability — the very reason this ledger exists — means no script can verify equivalence in general; the character floor is a substance proxy standing in for the judgment call, not a truth check. A reviewer recomputing the packet judges the argument itself. Expect the excusal rate to land inside the 4–39% band from the research; treat a rate far above it as laundering pressure, not bad luck.
...
Every id in the run's survivor list has either (a) a ledger entry that passes the gate or (b) a confirmed kill recorded by the re-run hunt; `check-excusals.py` exits 0 against the final survivor-list/ledger pair; and both files are captured in the evidence packet at the run's head. Anything short of that leaves the mutation gate red — report it red; a score reported as 100% with an unexcused survivor is laundering.
