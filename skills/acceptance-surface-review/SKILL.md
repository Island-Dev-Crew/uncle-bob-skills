...
Declare the surface in a manifest that sits beside the story. Four keys, one per line; paths are relative and resolve against the manifest's own directory, never the cwd and never outside it:
...
One resolver, [`declared_path()`](scripts/surface-check.py), is the only place a declared value becomes a path, and the only place scope is enforced. An absolute value is refused as malformed rather than silently rerouted: it ignores the manifest's directory, and it would judge different files on different machines. **A relative value that climbs out with `..` is refused on the same terms.** The manifest is contributor-authored text; a `..` entry adds a file the caller never put in scope while the gate still prints `surface complete` — which it did, exit 0, until this round. Containment is resolved rather than textual, so a symlink pointing out of the root is refused exactly like a literal `..`, and a `p/../p` that lands back inside still passes. **It is not a sandbox, and one shape it cannot see is a hard link (or a bind mount): that is a real directory entry inside the root, `resolve()` returns an in-root path for it, and the gate accepts what its symlink twin is refused for** — the same limit `leak-scan` states for the same mechanism, disclosed here because a limit only a reviewer knows about is not disclosed at all. The root defaults to the manifest's own directory — the tightest one, and the one paths already resolve against, so no flag is needed for the ordinary case. `--root DIR` adds another, repeatably, for the monorepo where story and implementation legitimately sit in different trees; a root that is not an existing directory is a usage error, never a narrower scope that happens to pass.
...
**Red/green proof.** The gate earns its `enforced` claim by having been watched failing: the [`known-dirty-fixture`](../known-dirty-fixture/SKILL.md) ritual. All eleven fixtures ship beside it, ten gates and one **disclosed limit** — the fixture that ships passing on purpose. Then come the two seal probes, which are runs rather than fixtures because no file can carry a dead pipe. Recompute from this island's directory, and read each exit code with `rc=$?` on the line after the run, never through a pipe. The seal probes print the child's own return code instead, for that same reason:
...
The eleventh came from an outside seat reading this island cold. **escaped-code** is honest in every visible way — tier `critical`, real Gherkin, real QA — and its `code:` entry is `../surface-check.py`, one directory up. The build that shipped ten fixtures read that outside file and printed `ok … surface complete`, exit 0: a contributor had widened the reviewed surface past the scope its caller declared, and the gate agreed. It now exits `2`. The second run is the same manifest with `--root scripts`, and it must keep exiting 0 — a containment rule that costs a two-tree repo its pass would push every such team straight back to no root at all.

The two runs after them are not fixtures, because a dead pipe cannot be checked into a directory. Each hands the gate a stream whose reader is gone, and each exited `120` against the build that shipped the first ten fixtures: a usage error and a clean verdict both wearing a code this island's table never named. Deleting any fixture, or either probe, returns the gate to `unverified`.
