...
- The evidence for keeping live judgment out of the gate: multi-app computer-use agents still succeed on only ~12–20% of tasks — an 80%+ failure rate — per the research brief ([atdd-gherkin-agile](../../research/atdd-gherkin-agile.md), "UI gates need determinized scripts, not raw agent judgment"). The seat's intelligence is spent once, at generation time; the artifact it leaves behind runs dumb, fast, and repeatable.

## Boundary: generation here, assertion mechanics there

Runtime UI evidence mechanics — the entrypoint contract, checkpoint and assertion patterns, stable locators, anti-flake rules, evidence capture, sandbox safety — come from [`computer-use-smoke`](../../COMPANION.md#computer-use-smoke) **by invocation**. This seat GENERATES the executable from the QA doc and binds it to the story; it never re-owns behavioral assertion patterns. Generate scripts that conform to that island's protocol (named checkpoints, one coded assertion each, exit code as the verdict) and let its enforced preflight do the runtime policing.
...
3. **Bind.** Stamp the script header with the story id, the QA doc path, and the QA doc's sha256, so the gate is traceable to the exact procedure it encodes:
...
4. **Prove red.** Run the script against a state where one expected outcome is broken (the pre-implementation build, or one deliberately falsified expectation) and capture the non-zero exit. A gate that cannot go red is not a gate.
...
- Binding: [`scripts/qa-bind-check.sh`](scripts/qa-bind-check.sh) exits non-zero when the `STORY` / `QA-DOC` / `QA-SHA256` headers are missing or the hash is stale, so an unbound or out-of-date script cannot pose as the story's gate.
- Runtime preflight and the exit-code verdict: owned and enforced by `computer-use-smoke` when invoked.

**Red/green proof of the binding gate.** Its own known-dirty pair ships beside it ([known-dirty-fixture](../known-dirty-fixture/SKILL.md)): one QA doc, two candidate gates differing only in their `QA-SHA256`. Run from this island's directory — recompute rather than trust:
...
**Advisory** at v0 — required by this doc, blocked by no hook yet; a later wave can add checkers:
