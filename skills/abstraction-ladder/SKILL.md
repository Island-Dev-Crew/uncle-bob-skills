...
Two honesty notes before the argument leans on any of it. The four-rung history and the identical panic are **the conversation's claim, cited as such** — this pack ran no historical survey to confirm them, and this island does not pretend otherwise. And Bob's framing line, *"software is the most complicated thing that humans have ever attempted"*, is attributed by him to Dijkstra and is recorded in the ledger as **unverified as a verbatim Dijkstra quote**; the nearest sourced ground is "intellectual manageability" in EWD340, 1972 ([seventies-canon](../../research/seventies-canon.md)). Neither note weakens what follows, because what follows rests on a mechanism, not on the anecdote.

## Why the panic was wrong the same way

Brooks supplies the mechanism the ladder story only illustrates. *No Silver Bullet* splits a system's difficulty into **accidental** complexity — the difficulty our representations and tools impose — and **essential** complexity, the difficulty of the thing itself. The research brief's reading of it is the load-bearing sentence here: agents shrink accidental complexity only, and deciding what the system *means* stays human ([seventies-canon](../../research/seventies-canon.md)).
...
The reason is not nostalgia, and the conversation names the payoff precisely. Asked how he knew his agents were failing, Bob's answer was recognition, not instrumentation: *"I could see the agent struggle and I recognized the struggle since I have been through that struggle… the novice would come in and not recognize the struggle"* (C27). Rung-below experience **is** the diagnostic instrument. A director who has never written the thing by hand has no reference signal for what struggling looks like from the inside.
...
**1. Is the cost you are cutting the value, or the labor?** If the practice got expensive because it *bores* a human, the new rung did not obsolete it — it made it cheap. That is the pack's founding move run backwards: CRAP and mutation testing were shelved on labor cost alone and came back the moment the labor went to zero, because *"these guys are fast and they don't care how boring the work is"* (C8, [`boredom-dividend`](../boredom-dividend/SKILL.md)). Cutting a tedious-but-right practice at the exact rung that made it free is the most expensive move on this page.

**2. What failure was it catching?** Name one, from the last year, concretely. Three honest answers exist: it caught this (keep, or replace deliberately); it caught nothing and the risk is real (it was catching silently — check before cutting); it caught nothing and the risk is gone (a genuine cut).

**3. What catches that failure now?** Name the tool, gate, or practice that inherits the job. "The agents won't make that mistake" is an assertion with no measurement behind it, and a rule kept alive by assertion is the exact breach [`values-not-disciplines`](../values-not-disciplines/SKILL.md) exists to rule on.

**4. Where is the note?** This one is answered *after* the cut, not before — it is what finishes a cut, never what permits one. Write the cut down beside the rules file it left: what was removed, the date, which of the three answers to question 2 applied, and what took over. This is the closing law made mechanical — the year-later rediscovery should cost a `grep`, not an outage.

**The ruling.** Cut only when (1) says value-not-labor, (2) names a real answer rather than a shrug, and (3) names the successor. Fail any of the three and the practice is **shelved with the failing reason recorded**, not cut — which is precisely the form `boredom-dividend` needs to find it again when the labor cost moves.

## Done when

- [ ] The rung-below weekend has a date on a calendar and a one-sentence writeup from the last one.
...
- `advisory` — **everything this island teaches.** The ladder argument is a historical prior, and the discipline test is judgment written down. This island **ships no script by design**: a gate over "was this cut justified" would be a thin wrapper reading back the prose its own user wrote, which measures compliance with a form and not the quality of a decision. Saying so is the honest option under the pack's second law ([CONTEXT.md](../../CONTEXT.md)), and the weekend cadence number in Practice one carries its own `unverified` marker at the line that sets it.
