...
4. **Repair on a breach** (advisory). `UNDER` has exactly two honest repairs — book strategic work into the next cycle, or attach the missing evidence to work already done — and re-run. Faking the share by re-tagging tactical rows is the failure this gate exists to catch; the evidence rule is what makes it expensive.

`OVER` the ceiling prints and exits 0. Over-investment is a judgment call — a deliberate paydown cycle after a structural collapse is correct, not a defect — so the ceiling is a reading and the floor is the gate.

## Enforced vs advisory (v0, stated honestly)

- **Enforced today:** the arithmetic and the verdict. [`scripts/strategic-share.py`](scripts/strategic-share.py) computes the evidenced share by cross-multiplication (no division, so the floor is exact at the boundary), rejects any tag outside `tactical|strategic` on a fully anchored match, enforces the `metric=before->after` form and the no-delta discount, keeps `#`-leading work items in the accounting and counts every line it skips, exits 1 on a breach, 2 fail-closed on ledger content, and 3 on IO or usage. Codes 2 and 3 are deliberately distinct: a mistyped path exiting 2 would read exactly like a legitimate fail-closed verdict. The island's own shape is enforced by the pack validator (`scripts/validate-island.py` at the pack root).
...
- **The known hole — name it, pair it.** The script checks that a metric **moved**, never that it moved the *right way*: `crap_max=5->14` is a real delta and counts. Direction is metric-specific (complexity down, coverage up), so the reviewer owns it — and the cheap pairing is to run the metric's own gate in the same cycle, where [`crap-gate`](../crap-gate/SKILL.md) exits non-zero on a ceiling breach and a regressed row cannot sit green beside it. A ledger scored without its metric gate must say so in its evidence.

**Red/green proof (run, not asserted).** The gate ships the fixture pair that proves it can fail — [`scripts/fixtures/dirty-tactical-drift.tsv`](scripts/fixtures/dirty-tactical-drift.tsv) (120m of 840m *claimed* strategic, a plausible-looking 14.3%, but one claim carries `crap_max=6->6`, so the evidenced share falls to 7.14%) and [`scripts/fixtures/clean-at-floor.tsv`](scripts/fixtures/clean-at-floor.tsv) (100m of 1000m, all evidenced). Run from this island's directory:
...
Both exit codes were observed on the shipped fixtures. The dirty fixture parses cleanly and fails on the *claim*, not on syntax — that is what makes it proof of this check rather than of the parser. The clean fixture lands **exactly on the 10% floor**, so the pair also proves the gate discriminates at the boundary instead of rejecting everything. The pack ritual ([`known-dirty-fixture`](../known-dirty-fixture/SKILL.md)) agrees, from the pack root: `skills/known-dirty-fixture/scripts/prove-gate.sh skills/strategic-ledger/scripts/fixtures/dirty-tactical-drift.tsv skills/strategic-ledger/scripts/fixtures/clean-at-floor.tsv -- python3 skills/strategic-ledger/scripts/strategic-share.py` → `ACCEPTED`, exit 0. Deleting either fixture returns this gate to `unverified`.
