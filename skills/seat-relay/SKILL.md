...
- **Seat mandates come from [`delegated-authority-prompt`](../../COMPANION.md#delegated-authority-prompt).** Each seat runs unattended, so compose its mandate as a front-loaded authority prompt with explicit stop conditions. That island is user-invoked too: no agent can fire it by description. Read its SKILL.md directly at the link and apply the pattern, or have the human run it. **It is not in this pack** — it is a Forge island, and this is a hand-off rather than a boundary statement. The link reaches a summary in [COMPANION.md](../../COMPANION.md#delegated-authority-prompt) naming the five slots; the island that composes them ships elsewhere, and this repository pins no URL to it. On a pack-only clone you write the mandate yourself against those five slot names.
...
- **Seat isolation reuses [`worktree-fleet`](../../COMPANION.md#worktree-fleet) mechanics where files are mutated in parallel.** Seats run serially within one story. When several stories are in flight and more than one coder mutates the same repo, give each its own worktree. That island is also user-invoked, so read it directly. Its evidence boundary carries over to batons produced in worktrees: worktree artifacts are inadmissible as gate evidence until re-derived from a fresh clone. **It is not in this pack either** — a conditional hand-off, reached only once a second coder touches the same repo. The evidence rule in the previous sentence is inlined here deliberately and holds with or without that island; the worktree mechanics are yours to supply on a pack-only clone.
...
1. **Compose the mandate** for the next seat: its input baton, its exit gate, its stop conditions (the delegated-authority-prompt pattern, read at the link above — a Forge island this pack does not ship, so on a pack-only clone this step is written by hand against the five slot names).
