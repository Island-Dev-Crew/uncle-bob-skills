...
- **Same-machine isolation mechanics belong to [`worktree-fleet`](../../COMPANION.md#worktree-fleet).** Once this island says "three coders", giving each its own worktree is that island's job. So is its rule that worktree artifacts are inadmissible as gate evidence until re-derived from a fresh clone.
...
- **A crowd of n peers** on shared-context work is a mesh: `n(n-1)/2` paths. Five peers is ten reconciliations, and every one of them is a chance to disagree.
- **A relay of n seats** is a chain: `n-1` handoffs. Five seats is four batons. Each baton is a *typed artifact* (Gherkin, code, CRAP report, mutation log, QA verdict), not a conversation, so the coordination cost per edge is bounded by what the artifact contains rather than by how long two agents argue (C9).
