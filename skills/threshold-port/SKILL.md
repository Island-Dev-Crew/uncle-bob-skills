...
Bob's sharpest line in the conversation draws the whole map: *"it's probably a mistake to impose a human discipline on an agent. It is not a mistake to impose human values on the agent, but there may be thresholds that we need to change"* (C17, [ledger](../../01-CONCEPT-LEDGER.md)). This island turns that line into a repeatable port. Any human practice — TDD, coverage floors, complexity caps — crosses to the agent lane in three moves, and the number at the end moves only on evidence.

## Scope: numbers, not markdown

This island targets **numeric gate parameters** — complexity caps, coverage floors, CRAP cutoffs, mutation budgets — never markdown files. Tuning a skill, a CLAUDE.md, or any prose context file is [`skill-tune`](../../COMPANION.md#skill-tune)'s lane entirely. Likewise the experimental engine this island leans on — the measure-edit-keep-iff-score-moved loop — **belongs to skill-tune and is referenced here, never re-derived**. Run that loop with the gate level as the one edited variable and lane outcomes as the score; this island supplies what to keep, what to drop, and what counts as a win for a *number*.

## Move 1 — keep the value

Name what the practice actually protects, as a property a tool can measure: tested, covered, small, clean. Values transfer intact (C17). A port whose value resists measurement is not ready for move 3 — carry it as advisory prose and say so, until a measurable proxy exists.
...
The canonical case (C17): strict TDD line-interleave, which Bob explicitly does not impose — *"I don't think it makes any sense to make an agent write a single line of a test and then write a single line of the production code."* Agents naturally *"write a function and then write the test for that function"*, and *"They always fall back on doing that… So I figure that's probably okay."* The value — every path covered by a test — crosses whole; the red-green rhythm stays home.

## Move 3 — retune the threshold by controlled experiment

Human-calibrated numbers carry human assumptions, so the ported level starts as a hypothesis. Bob's own path is the template (C17): *"for a human I would keep crap numbers below four… but for the agents I've set this at six and… maybe I'll push it to eight."* The number-lane instantiation of skill-tune's loop:
...
2. **Change one level** — one variable, one step (4→6, then 6→8; a 4→8 jump makes the delta unattributable).
...
Bob justifies the looser agent level by *"a huge short-term memory and a perfectly accurate short-term memory"* (C17). That is his **hypothesis**, not established research — [`research/crap-metric.md`](../../research/crap-metric.md) flags it unverified. Treat the rationale exactly like the number: if agents thrash at 8, the hypothesis lost, however good the story. Outcomes outrank rationales in both directions.
