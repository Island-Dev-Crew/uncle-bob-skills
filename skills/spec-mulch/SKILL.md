...
- **Explicit divergence from spec-pipeline.** [`spec-pipeline`](../../COMPANION.md#spec-pipeline) Stage 1 ends with the spec *published* where the repo keeps specs, a durable artifact. This pack diverges from that published-spec rule, deliberately and by name: under the mulch doctrine, a published spec has a lifespan of exactly one story. Publish it to launch, mark it to die, delete it on merge. Everything else about Stage 1 stays spec-pipeline's; only the artifact's fate is overridden here.
...
**The spec launches the story and dies. What merged, plus the gates that hold it, is the specification.**
