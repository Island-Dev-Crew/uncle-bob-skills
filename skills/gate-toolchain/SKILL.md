...
typescript	mutation	stryker	--incremental	./node_modules/.bin/stryker run --incremental
```

[`scripts/toolchain-check.py`](scripts/toolchain-check.py) reads that manifest and enforces exactly four claims per row. The tool is the mapped implementation for that language and gate. The command names that tool. The run is really diff-scoped: the declared flag is flag-shaped, present in the command, not negated, and no whole-repo flag sits in either column. And the command reaches its binary locally: **a fetch-and-run launcher is rejected**, because a row that says `npx stryker` resolves an unpinned package off a registry and executes whatever answers, which is a supply-chain decision wearing a gate's clothes. The launcher is matched on its basename, so path-qualifying it (`/usr/bin/npx`) is the same refusal as the bare name, and a long spelling (`bun x`, `uv tool run`) is refused with the alias it expands. Declare the path your lockfile pins (`./node_modules/.bin/stryker`, `mvn`, `cargo`), or pin the launcher with `--no-install`/`--offline`.
...
**Red/green proof.** All seven fixtures live beside the script. Recompute from this island's directory; these thirteen commands gave these exit codes:
...
python3 scripts/toolchain-check.py scripts/fixtures/remote-runner.tsv         # exit 1 — 11 declarations, 8 breaching
...
# the two paths that used to exit 120, plus the interrupt — returncode captured, not piped, and re-raised as the wrapper's own exit so the pack verifier re-runs each row:
python3 -c 'import os,subprocess,sys;r,w=os.pipe();os.close(r);sys.exit(subprocess.run([sys.executable,"scripts/toolchain-check.py","--nope"],stderr=w,stdout=subprocess.DEVNULL).returncode)'    # exit 2 — usage error onto a dead stderr, not 120
python3 -c 'import os,subprocess,sys;r,w=os.pipe();os.close(r);sys.exit(subprocess.run([sys.executable,"scripts/toolchain-check.py","--help"],stdout=w,stderr=subprocess.DEVNULL).returncode)'    # exit 2 — --help onto a dead stdout, not 120
python3 -c 'import signal,subprocess,sys,time;p=subprocess.Popen([sys.executable,"scripts/toolchain-check.py"],stdin=subprocess.PIPE,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(0.6);p.send_signal(signal.SIGINT);sys.exit(p.wait())'  # exit 130 — the shell convention for SIGINT, by normal exit, never a verdict
```
...
**One disclosed limit, fixtured not fixed.** `limit-space-separated.tsv` exits 0 on `./node_modules/.bin/stryker run --incremental false`. The scope reads present because negation is only read in its `=`-valued spellings, and a bare `false` in the next token is as often a positional argument as a value. Widening the rule would buy this case at the price of false reds on `--in-diff 0`-shaped arguments, so the claim is narrowed instead: verify a space-separated boolean against your tool once, the way the advisory below already asks you to verify the flag exists.
...
- `advisory` — **that the flag you declared exists in the tool's CLI, and that the named tool is the process that runs.** The gate checks the flag is flag-shaped and really in your command; it cannot know your tool's option set. A declared `--diff` that the tool ignores passes the gate and fails reality, so verify the flag against the tool once, then let the gate hold it. The tool check likewise compares names with case and separators ignored. It is anchored to whole segments but matched across token joins, so `cargo mutants` satisfies `cargo-mutants` and `org.pitest:pitest-maven` satisfies `pitest`: it catches a command that never mentions the tool, not one that mentions it in passing (`echo mutmut --incremental` passes). Whole-repo spellings outside `DENY_SCOPE` (`--full-run`) still pass in both columns, because the deny-list is a named set, not a semantic judgement. Negation outside its `=`-valued spellings passes too, per the fixtured limit above. The launcher rule is a named set on the same terms: a launcher outside it, one reached through a wrapper script or a shell alias of your own, or a shell that resolves one indirectly (`$RUNNER stryker`), passes. A command field is free text and no token test reads every spelling. Basename comparison closes the path spelling and only that: a Windows path spelled with backslashes is resolved as shell escapes *before* the comparison, so `C:\tools\npx.cmd` still passes where its forward-slash twin is refused. It fails closed in the other direction — a space-separated path argument whose own basename is a launcher name (`--in-diff build/npx`) reads as a launcher, though the attached spelling `--in-diff=build/npx` does not. And the `--no-install`/`--offline` pin is read as present in the command, not as bound to the launcher it should govern.
...
- [ ] Every command invokes the binary your lockfile pins, not a launcher that would fetch one
