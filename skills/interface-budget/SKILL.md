...
- **`kind`** is `interface`, `test`, or `impl`. There is no fourth kind; an unrecognised label is rejected rather than waved through.
...
[`scripts/load-ledger.py`](scripts/load-ledger.py) reads the ledger and returns a verdict on four rules: `IMPL-FIRST` (an implementation opened before this task's first interface or test row), `UNJUSTIFIED` (a tier-3 reason outside the vocabulary), `RE-READ` (one body bought twice at the same tier or below), and `OVER-BUDGET` (implementation share above the declared ceiling; breach is strictly greater, exactly-at passes — and the printed share is rounded **up** to basis points, so a share that breached can never print as equal to the ceiling it breached). Exit codes are separated by meaning: `0` pass (and `--help`, which gates nothing), `1` verdict, `2` usage, IO, closed or detached stdin, non-UTF-8, malformed, empty ledger, or a report that cannot be written — stdout closed at startup, closed by the reader mid-report, or otherwise unwritable. Even an unhandled crash is caught and re-coded `2` rather than left to CPython's exit `1`. So is every `SystemExit` the script or argparse raises, `--help` and a usage error included — because an exit that sails past that seal into interpreter shutdown meets the flush CPython runs there, and when that flush fails on a dead stream CPython **discards the status and substitutes its own `120`**, a code in no table here. The streams are therefore flushed while the code is still the script's to set, and a stream that will not flush is pointed at `/dev/null` (or, if even that fails, the process leaves by `os._exit`, which runs no shutdown at all). A ledger the gate cannot read is never a verdict — `1` is reserved for real violations, so a fix-until-green loop is never handed an unreadable file as if it were repairable rows. Signals sit outside that table and are named rather than hidden: none is *handled* — the one clause naming `SIGINT` re-raises it untouched — so it kills the process with signal 2, which a shell reports as `130`.
...
Six more forged rows, each a spelling that used to walk past `RE-READ` and now cannot be logged at all (the last is the reverse case — a spelling that used to fail and now gates) — plus the seven ways the *process* can fail without a ledger being at fault, which must borrow neither `1` nor `120`, and an eighth that only looks like one. The `→` annotation is the gate's own exit code, re-captured for every row: by `cmd; rc=$?` on its own line with stdin redirected from a file rather than read after the pipe that would clobber it, or — for the rows that must forge a dead stream — by the child's own `returncode`, printed as the row's single output line:
...
- The clean file's `T-77` sits at exactly 35.00%, so the gate discriminates at the ceiling instead of rejecting every implementation load, and its `T-79` is the single-file module — `src/tax/rate.py` read as `interface` and then as `impl` under `interface-silent` — which proves the prescribed tier escalation stays green.
...
- `enforced` — the four rules and the verdict: `load-ledger.py` rejects unknown kinds and non-integer token counts, lowercases `kind` and `reason` so casing cannot evade the impl check, casefolds the `task_id` half of the `RE-READ` key, folds separators, the `.`/`..`/`//` family, letter case and Unicode form into the path half, and refuses at parse every spelling it cannot fold — non-repo-relative (absolute, drive-qualified, home-anchored, `..`-escaping) and invisible-character paths alike — ranks the three kinds so a legal tier escalation is not charged as a re-read, decides `OVER-BUDGET` in integer basis points rather than float comparison and prints the share rounded up so the message cannot contradict the verdict, cites findings by the path as given rather than by basename, anchors the comment marker at column 0 so no indented row is silently dropped, and exits `2` fail-closed on an empty, malformed, or un-decodable ledger, on a closed or detached stdin, on a report it cannot write, and on any unhandled crash — **no failure this script can raise borrows `1`**, and none reaches interpreter shutdown with a stream that could still fail there, so none borrows CPython's `120` either; the only exits outside its table are signals (`SIGINT` → `130`). The island's own shape is enforced by the pack validator (`scripts/validate-island.py` at the pack root).
- `advisory` — everything the gate reads but cannot witness. **The ledger is a self-report.** Nothing here proves a row's `kind` label is truthful, that `tokens` is honest, that a logged reason was the real motive, or that two rows filed under one `task_id` were one task; a seat that mislabels an implementation as `interface` passes a gate it has already defeated. The path fold is **textual only** — the gate never stats the filesystem, so two different paths reaching one body through a symlink or hard link stay two keys and the over-read between them passes. The ceiling number, the tier ordering as a habit, and the read-the-tests-first heuristic are advisory for the same reason. Making the ledger tamper-resistant means **generating it from the harness tool-call transcript instead of from the seat's own account** — that generator is not shipped here, and until it is, this island's honest claim is that it gates *the accounting*, not *the behaviour*.
...
- [ ] The ledger's provenance stated — self-reported (advisory) or harness-generated (stronger) — never left implied ([CONTEXT.md](../../CONTEXT.md))
